...
By default the challenger is paired per (fold, seed) against the **upper
envelope of the two shipped baseline arms** (the per-cell max over ff/lstm:
a conservative, test-selected bar, not a runnable model). If your model
belongs to a declared
...
an external challenger) lives in `examples/demo_submission/` with its
committed claim block, generated by the bare default command (envelope
pairing, dMCC $-0.0066$ --- the same block the paper's Section 6 prints).
Like-for-like against the ff arm (`--baseline-arch ff`) reads $+0.0033$;
both are within the reference null.
